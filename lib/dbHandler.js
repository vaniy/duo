...
var checkUserExists = function(openId, data, req, callBack) {
...
var createUser = function(data, preLevel) {
    ...
    mongoClient.connect(mogoUrl, function(err, db) {
        var cursor = db.collection('order').find({ "openId": req.query.openId }).toArray(function(err, doc) {
            if (err) {
                ...
                res.send({ status: 'succeed', data: doc });
                db.close();
            }
        });
    });
    ...
    mongoClient.connect(mogoUrl, function(err, db) {
        var cursor = db.collection('clock').find({ "openId": openId }).toArray(function(err, doc) {
            if (err) {
                ...
                res.send({ status: 'succeed', persons, count, data: {} });
                db.close();

            } else {
                res.send({ status: 'succeed', data: doc[0], persons, count });
                db.close();
            }
        });
    });
    ...
    mongoClient.connect(mogoUrl, function(err, db) {
        var cursor = db.collection('clock').find({ "clockCount": { $gt: 0 }, "clockTime": day }).sort({ reduceWeight: -1 }).toArray(function(err, doc) {
            if (err) {
                ...
                // res.send({ status: 'succeed', count: 0, persons: [] });
                ...
                // res.send({ status: 'succeed', count: doc.length, persons });
                db.close();
            }
        });
    });
